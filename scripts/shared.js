...
/* Confirms before handing the reader off to another site. Driven by the
   ...
   the destination stays declared in the markup — the article page marks up its
   links the same way. Falls through to a plain link wherever <dialog> is
   unsupported, or on a page that carries no dialog. */
function setupExitConfirm() {
    ...
    /* Every position here comes from a pointer, so it is in viewport coordinates —
       but style.left is resolved against the dialog's containing block, and the
       root's reserved scrollbar gutter (scrollbar-gutter: stable both-edges) sets
       the two apart by the gutter's width. Measured once per open rather than
       assumed, since it is zero wherever scrollbars are overlays. */
    let origin = { x: 0, y: 0 };
    const measureOrigin = () => {
    ...
    const moveTo = (left, top) => {
        const box = dialog.getBoundingClientRect();
        const x = Math.max(EDGE, Math.min(left, window.innerWidth - box.width - EDGE));
        const y = Math.max(EDGE, Math.min(top, window.innerHeight - box.height - EDGE));
        dialog.style.left = `${x - origin.x}px`;
        dialog.style.top = `${y - origin.y}px`;
    };
    ...
    document.addEventListener('click', (event) => {
        ...
        dialog.showModal();
        measureOrigin();
        /* Opens off the pointer, the way a context menu does — on a long link,
           or one wrapping onto two lines, the far corner of its box can be a long
           way from where the reader actually clicked. A keyboard activation has no
           pointer to read: it reports detail 0 and 0,0 coordinates, which would
           park the dialog in the top-left corner, so those keep using the link. */
        if (event.detail > 0) {
            moveTo(event.clientX + EDGE, event.clientY + EDGE);
        } else {
